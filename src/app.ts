...
async function buscarDashboard(): Promise<RespostaDashboard> {
...
function somarPrecos(servicos: Servico[]): number {
    return servicos.reduce((acumulador, servico) => acumulador + servico.preco, 0);
}


function calcularMediaPrecos(servicos: Servico[]): number {
...
function encontrarServicoMaisCaro(servicos: Servico[]): Servico | null {
    if (servicos.length === 0) {
        return null;
    }
...
function filtrarServicosPremium(servicos: Servico[]): Servico[] {
    return servicos.filter((servico) => servico.preco > 300);
}


function filtrarAgendamentosAtivos(agendamentos: AgendamentoResumo[]): AgendamentoResumo[] {
    return agendamentos.filter((agendamento) => agendamento.status !== 'cancelado');
}


function calcularFaturamentoPrevisto(ranking: ServicoRanking[]): number {
...
function calcularFaturamentoRealizado(ranking: ServicoRanking[]): number {
    return ranking.reduce((total, servico) => {
...
function contarAgendamentosPorServico(agendamentos: AgendamentoResumo[]): Record<string, number> {
...
function encontrarServicoMaisAgendado(contagem: Record<string, number>): string {
    let nomeDestaque = '';
    let maiorTotal = 0;
...
function formatarRanking(ranking: ServicoRanking[]): LinhaRankingFormatada[] {
...
function renderizarCardsServicos(servicos: Servico[]): void {
...
function renderizarCardsDashboard(dados: RespostaDashboard): void {
...
function renderizarRanking(linhas: LinhaRankingFormatada[]): void {
