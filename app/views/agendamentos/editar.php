...
if ($idAgendamento < 1) {
    header('Location: /dashboard?naoencontrado=1');
    exit;
}
...
$valores = [
    'id_cliente' => (string) $agendamento['id_cliente'],
    'id_profissional' => (string) $agendamento['id_profissional'],
    'id_servico' => (string) $agendamento['id_servico'],
    'data_hora_servico' => date('Y-m-d\TH:i', strtotime($agendamento['data_hora_servico'])),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($erros)) {
    ...
    if (empty($erros)) {
        try {
            atualizarAgendamento($pdo, $idAgendamento, [
...
?>
    ...
    <header class="admin-topbar">
        <div>
            <h1 class="admin-topbar-titulo">Editar Agendamento</h1>
            <p class="admin-topbar-subtitulo">
                Alterando o agendamento de <?php echo htmlspecialchars($agendamento['nome_cliente']); ?>
            </p>
        </div>
    </header>
    ...
    <div class="admin-container">
        ...
        <div class="superficie p-4 p-md-5">
            <form method="POST" action="/agendamentos/editar?id=<?php echo $idAgendamento; ?>" data-parsley-validate="">
                <div class="row g-3">
                    <div class="col-md-6">
